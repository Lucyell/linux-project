...
void ProgramManager::schedule()
{
    ...
    else if (running->status == ProgramStatus::DEAD)
    {
        ...
        if (parent == nullptr || parent->status == ProgramStatus::DEAD)
        {
            releasePCB(running);
        }
        else
        {
            running->status = ProgramStatus::ZOMBIE;
        }
    }
...
void ProgramManager::exit(int ret)
{
    ...
    // 将所有子进程的父进程设置为0（无父进程，内核直接管理）
    ListItem *item = this->allPrograms.front();
    while (item != nullptr)
    {
        PCB *pcb = ListItem2PCB(item, tagInAllList);
        item = item->next;

        if (pcb->parentPid == program->pid && pcb->status != ProgramStatus::DEAD)
        {
            pcb->parentPid = 0;  // 标记无父进程，内核直接管理孤儿
            printf("[exit] orphan %d set to have no parent (orphan)\n", pcb->pid);
        }
    }
...
bool ProgramManager::isProcessAlive(int pid)
{
    for (ListItem *item = this->allPrograms.front(); item != nullptr; item = item->next)
    {
        PCB *pcb = ListItem2PCB(item, tagInAllList);
        if (pcb->pid == pid)
        {
            return pcb->status != ProgramStatus::DEAD;
        }
    }
    return false; // 找不到进程，认为不活跃
}


void ProgramManager::freeProcessResources(PCB *program)
{
    this->allPrograms.erase(&program->tagInAllList);

    // 从就绪队列和其他队列移除
    this->readyPrograms.erase(&program->tagInGeneralList);

    // 释放内存相关（和 exit 中的释放相似）
    int *pageDir, *page;
    int paddr;
...
int ProgramManager::wait(int *retval)
{
    ...
    while (true)
    {
        ...
        while (item)
        {
            ...
            {
                flag = false;
                if (child->status == ProgramStatus::DEAD || child->status == ProgramStatus::ZOMBIE)
