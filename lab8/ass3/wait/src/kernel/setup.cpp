...
// 新增函数
void auto_reap_test(void *arg)
{
    int pid = fork();
    if (pid > 0)
    {
        printf("[auto reap test] parent exiting, pid=%d\n", programManager.running->pid);
        exit(333);
    }
    for (volatile int i = 0; i < 300000; ++i);
    printf("[auto reap test] child still running, parentPid = %d\n", programManager.running->parentPid);
    //exit(777);
...
extern "C" void setup_kernel()
{
    ...
    // setup_kernel() 中新增：
    int pid_auto_reap_test = programManager.executeProcess((const char *)auto_reap_test, 1);
    printf("[setup] created auto_reap_test thread, pid=%d\n", pid_auto_reap_test);
